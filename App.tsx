...
// Fix: Import generateQuiz from the services layer
import { generateQuiz } from './services/gemini';

type AppScreen = 'home' | 'mode_select' | 'quiz_setup' | 'quiz' | 'results' | 'notebook' | 'profile';
...
const App: React.FC = () => {
        ...
        return (
          <QuizSessionUI 
            ...
            onComplete={(results, time) => {
              setQuizResults({ answers: results, time });
              navigateTo('results');
            }}
  ...
  return (
    <div className="flex flex-col h-screen max-w-md mx-auto bg-slate-50 relative shadow-2xl overflow-hidden">
      ...
      {/* Bottom Navigation */}
      <nav className="absolute bottom-0 w-full bg-white border-t border-slate-200 flex justify-around py-3 px-2 z-50">
        <button 
          onClick={() => navigateTo('home')}
          className={`flex flex-col items-center gap-1 ${currentScreen === 'home' ? 'text-indigo-600' : 'text-slate-400'}`}
        >
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
          <span className="text-xs font-medium">題目</span>
        </button>
        <button 
          onClick={() => navigateTo('notebook')}
          className={`flex flex-col items-center gap-1 ${currentScreen === 'notebook' ? 'text-indigo-600' : 'text-slate-400'}`}
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center gap-1 ${currentScreen === 'profile' ? 'text-indigo-600' : 'text-slate-400'}`}
        >
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
          <span className="text-xs font-medium">個人檔案</span>
        </button>
      </nav>
    </div>
  );
...
// Internal component for Quiz Setup (Question Count)
